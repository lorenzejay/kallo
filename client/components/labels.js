...
const Labels = ({ task, projectId, setTask }) => {
  ...
  const handleAddLabelToTask = async () => {
    ...
    await axios.put(`/api/projects/add-column/${projectId}`, { columns: boardColumns }, config);
    setLabels(taskTags);
    setLabelColor("");
    setLabelName("");
  };
        ...
        <div className="flex items-center">
          ...
          <FaPlus
            className={`bg-gray-300 p-1 ${
              labelName === "" || !labelColor ? "disabled:opacity-50" : "bg-blue-500"
            } cursor-pointer rounded-md`}
            size={30}
            onClick={handleAddLabelToTask}
          />
        </div>
        <input type="color" onChange={(e) => setLabelColor(e.target.value)} value={labelColor} />
      </div>
